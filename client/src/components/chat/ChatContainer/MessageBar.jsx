...
import { PaperClipIcon, PaperAirplaneIcon, XMarkIcon, EyeIcon } from '@heroicons/react/20/solid';
import {useSelector, useDispatch} from 'react-redux'
import EmojiPick from '../../common/EmojiPick';
import { useSocket } from '../../../contexts/SocketContext';
import axiosInstance from '../../../services/axios';
import { CHAT_ROUTE, HOST } from '../../../utils/constants';
import {setIsUploading} from '../../../slices/ChatSlice'
...
function MessageBar() {
  ...
  const {selectedChat, selectedChatType} = useSelector(state => state.chatData)
  ...
  const handleFileChange = async (e) => {
    try {
      ...
      const response = await axiosInstance.post(`${CHAT_ROUTE}/upload-file`, formData, { withCredentials: true, headers: {
  ...
  const sendSocketMessage = (message) => {
    socket.emit(`${message.channelId ? 'send-channel-message' : 'send-message'}`, message);
    fileInputRef.current.value = null;
    setFileUrls([]);
    setMessage('');
  };
  ...
  const handleSend = () => {
    if (fileUrls.length > 0 && message.trim()) return;
    const baseMessage = { sender: user.id };
    if (selectedChatType === 'dm') {
      baseMessage.receiver = selectedChat._id;
    } else if (selectedChatType === 'channel') {
      baseMessage.channelId = selectedChat._id;
    }
    if (fileUrls.length > 0) sendSocketMessage({...baseMessage, messageType: 'file', fileUrl: fileUrls,});
    else sendSocketMessage({...baseMessage, messageType: 'text', content: message,})
  };  

  return (
    <div className="h-[10vh] bg-white border-t-2 border-indigo-200 flex justify-center items-center px-4">
      {fileUrls.length > 0 && <Drawer open={previewDrawerOpen} setOpen={setPreviewDrawerOpen} side='bottom' width="w-[100vw] md:w-[67vw]" height="h-[50vh] md:h-[50vh] max-h-[70vh]">
        <div className="px-4 py-2 flex gap-4 overflow-x-auto">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
            {fileUrls.slice(0, 4).map((fileUrl, index) => {
              ...
              return (
                <div key={index} className="relative border border-primary rounded overflow-hidden aspect-square group">
                  {isImage ? (
                    <img
                      src={`${HOST}/${fileUrl}`}
                      alt={`preview-${index}`}
      ...
      <div className="flex-1 flex items-center">
        <div className="flex flex-1 items-center bg-white border-2 border-indigo-200 rounded-md px-3 py-1 gap-3">
          ...
          />
          <EmojiPick handleEmojiClick={handleEmojiClick} emojiRef={emojiRef} open={open} handleEmoji={handleEmoji}/>
        </div>
        {fileUrls.length > 0 && <Tooltip
          content="New Channel"
          position="top"
          bgColor="bg-white"
          textColor="text-primary"
        >
          <button onClick={() => setPreviewDrawerOpen(true)} className={`rounded-lg transition `}>
        ...
        <button
          type="button"
          onClick={handleSend}
          disabled={!message && fileUrls.length === 0 || (message && fileUrls.length > 0)}
