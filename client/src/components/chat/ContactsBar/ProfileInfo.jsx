...
function ProfileInfo() {
  const { image, name, email, color } = useSelector((state) => state.userData.user);
  ...
  return (
    <div className="bg-white flex flex-1 flex-col items-center justify-center rounded-2xl p-6 w-full max-w-xs mx-auto shadow-lg">
      ...
      <div className="text-center w-full space-y-2">
        <div className="text-2xl sm:text-3xl md:text-4xl font-semibold text-primary">{name}</div>
        <div className="text-sm md:text-md lg:text-lg text-gray-700">{email}</div>
      </div>
      <Button
        label="Edit profile"
