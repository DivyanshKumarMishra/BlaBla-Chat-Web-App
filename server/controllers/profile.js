  ...
  async (req, res, next) => {
    ...
    if (!errors.isEmpty() && req.file?.path) {
      fs.unlink(req.file.path, async (err) => {
        if (!err) sendErrorResponse(res, errors);
      });
      return;
    } else {
      try {
        ...
        if (!name || !email || !color)
          throw { status: 400, message: 'Fill in all the details' };
        const user = await User.findById(req.userId);
        ...
        return res.status(201).json({
          ...
          profileSetup: updated_user.profileSetup,
          color: updated_user.color,
          id: updated_user._id,
          image: updated_user?.image?.replace(/\\/g, '/'),
        });
      } catch (err) {
